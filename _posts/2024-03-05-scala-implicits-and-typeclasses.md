...
В Scala для этой цели мы можем использовать те самые имплиситные классы, которые на самом деле подозрительно сильно похожи на адаптер, написанный выше. Отличие в том, что методы, для проверки на четность и нечетность сможем вызывать так, будто они принадлежат типу `Int`, что позволяет писать более простой и выразительный код.
...
Если мы избавимся от синтаксического сахара (в IntelliJ IDEA можно сделать `Desugar Scala Code`), то увидим, что в обессахаренном коде производится явное оборачивание в класс обертку и вызов его методов.

```scala
// Обессахаренный код
Example.RichInt(10).isEven  // true
Example.RichInt(10).isOdd   // false
```
...
![](assets/img/scala-implicits-and-typeclasses/mem2.png){: .w-50 .shadow }

Тайпклассы тоже позволяют добавлять в существующие типы функциональность (или поведение), причем делают это с подмешиванием полиморфизма в этот механизм.

Давайте начнем с рассмотрения абстрактного примера, где у нас есть классы `Circle` и `Rectangle`, в которые хотим добавить методы для расчета их площади. 
При использовании полиморфизма через наследование мы создаем интерфейс `Area` с методом `area` и наследуем от него классы `Circle` и `Rectangle`, в которых делаем реализацию метода для расчета площади.
...
def areaOf(area: Area): Double = area.area  // Generic function

areaOf(new Circle(10))
areaOf(new Rectangle(5, 5))
```

Это ООП подход, суть которого в сосредоточении данных и связанных с ними методов в одном месте - в определении класса.

Тайпклассы же предлагают другой подход — сущности, представляющие данные, отделены от сущностей, отвечающих за реализацию.
...
def areaOf[A](shape: A, area: Area[A]): Double = area.area(shape) // Generic function
...
def areaOf[A](figure: A)(implicit area: Area[A]): Double = area.area(figure) // Generic function
...
Таким образом, после рассмотрения этого примера, надеюсь понятно, что тайпклассы на самом деле можно реализовать и в других языках программирования, но в Scala они, за счет имплиситов, выглядят более изящно и выразительно.
...
Давайте рассмотрим эти компоненты поподробнее. Вот пример трейта, это собственно сам тайпкласс у которого есть некоторый метод. Важно обратить внимание, что этот трейт параметризован некоторым типом `A`.
...
Освоение имплиситов и тайпклассов является важным шагом на пути становления Scala разработчика. Однако, важно использовать их с умом. Правильно применяемые имплиситы и тайпклассы способствуют созданию чистого, безопасного и легко расширяемого кода, подчеркивая мощь Scala.